...
const NAME_MAP: Record<string, string> = {
  ...
  "추": "추가 피해", "추가피해": "추가 피해", "추가 피해": "추가 피해", "추피": "추가 피해",
...
export default function QuickAddModal({
  ...
  const selectBase =
    "w-full bg-[#22272e] border border-[#444c56] rounded-md px-2.5 sm:px-3 pr-8 sm:pr-10 py-1.5 " +
    "text-xs sm:text-sm text-gray-300 appearance-none transition-all duration-150 " +
  ...
  return (
    <div
      className="fixed inset-0 z-[1000] bg-black/40 flex items-center justify-center p-4 sm:p-6 md:p-8 font-sans"
      onDragOver={e => e.preventDefault()}
    >
      <div className="bg-[#2d333b] border border-[#444c56] rounded-2xl shadow-xl w-full max-w-lg sm:max-w-xl md:max-w-2xl">
        {/* 헤더 */}
        <div className="flex items-center gap-2 p-3 sm:p-4 border-b border-[#444c56]">
          <b className="text-sm sm:text-base md:text-lg text-gray-200">젬 추가</b>
          <div className="grow" />
          {/* <button className="text-gray-300 hover:text-white text-sm" onClick={onClose}>닫기</button> */}
        </div>

        {/* 상단 안내 */}
        <div className="flex items-center gap-2 px-3 sm:px-4 pt-2.5 sm:pt-3">
          {/* 
          <span className="text-xs sm:text-sm text-gray-300">계열 기본값</span>
          ...
          */}
          ...
          <div className="text-[11px] sm:text-xs md:text-sm text-gray-400">
            형식:{" "}
            <code className="text-gray-300">[계열] 의지력 포인트 [옵션1] [옵션2]</code>
          </div>
        </div>
        ...
        <div className="px-3 sm:px-4 pt-2.5 sm:pt-3">
          <textarea
            className="
              w-full h-40 sm:h-44 md:h-52 bg-[#22272e] border border-[#444c56] rounded-md
              px-2.5 sm:px-3 py-2 text-xs sm:text-sm text-gray-300
        ...
        <div className="px-3 sm:px-4 pb-2">
          {error ? (
            <div className="border border-amber-500/40 bg-amber-500/10 rounded-xl p-2 text-amber-200 text-xs sm:text-sm">
              {error}
            </div>
          ) : (
            <div className="border border-dashed border-[#444c56] rounded-xl p-2 text-center text-[11px] sm:text-sm text-gray-400">
        ...
        {/* 프리뷰 */}
        {preview.length > 0 && (
          <div className="px-3 sm:px-4 pb-2 space-y-1.5 sm:space-y-2 max-h-56 sm:max-h-64 overflow-auto">
            {preview.map((d, idx) => {
              const needDisplay = Math.max(1, d.base - d.effLv);
              return (
                <div key={idx} className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-[12px] sm:text-sm text-gray-200">
                  <span
                    className={`px-1.5 sm:px-2 py-0.5 rounded-full border ${
                      d.family === 'order'
                  ...
                  <span className="border border-[#444c56] rounded px-1.5 sm:px-2 py-0.5">의지력 {needDisplay}</span>
                  <span className="border border-[#444c56] rounded px-1.5 sm:px-2 py-0.5">포인트 {d.pts}</span>
                  <span className="text-[11px] sm:text-xs md:text-sm text-gray-400">(기본 {d.base}, 효율 Lv{d.effLv})</span>
        ...
        {/* 푸터 */}
        <div className="flex items-center gap-2 p-3 sm:p-4 border-t border-[#444c56]">
          <button
            className="px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-[#444c56] bg-[#2d333b] text-gray-200 hover:bg-[#30363d] transition text-xs sm:text-sm"
            onClick={onClose}
          >
            취소
          </button>
          <div className="grow" />
          <button
            className="px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-[#444c56] bg-[#2d333b] text-gray-200 hover:bg-[#30363d] transition text-xs sm:text-sm disabled:opacity-50"
